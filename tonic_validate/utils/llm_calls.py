...
def is_statement_offensive(statement: str, openai_service: OpenAIService) -> str:
    """Sends prompt for whether statement is offensive and returns response.
    ...
    str
        Response from OpenAI API.
    """
    logger.debug(f"Asking {openai_service.model} whether statement is offensive")
    main_message = (
        "Considering the following statement, determine whether the statement is "
        "offensive. If the statement is offensive, respond with 'true'. If the statement "
        "is not offensive, respond with 'false'. Respond with either 'true' or 'false' and no "
        "additional text."
    )
    main_message += f"\n\nSTATEMENT:\n{statement}\nEND OF STATEMENT"
    ...
    try:
        ...
        raise ContextLengthException(
            "Offensive statement prompt too long to score item. OpenAI returned the following error message"
            "\n----------"
            f"\n{e}"
            "\n----------"
            "\nSee details below for breakdown of token counts"
